import './App.css';

function App() {
  return (
    
   <div className='container'>


    <div className='leftcont'>
      
    <div className='left'>

    <img src='logo.png' alt='logo' className='logo' width={100} height={80}></img>

    </div>

    <div className='left1'>
      <div className='mid'>
        <h1 className='midtext'>Mid Century <br></br>Morden Furniture</h1>
        </div> 

        <div className='morden'>
        <p>Morden high quality and comfortable <br></br>available on our stores</p>
        </div>

        <div className='btn'>
        <button id='button'>Discover Now</button>
        </div>
        
      </div>

    <div className='left2'>
      <div className='scroll'>
        <p className='mscroll'>Scroll down</p>  
        <img src='mouse.png'></img>
    
      </div>

      <div className='chair1'>

        <img src='Chaiir1.jpg' alt='chair' className='chaiir1'></img>

      </div>
        
      </div>

    </div>
    <div className='centercont'>
    <div className='center'>
      <div className='uul'>
      <ul>
      <li><a href='about'>About Us</a></li>
      <li><a href='furniture'>Furniture</a></li>
        
      </ul>
      </div>
    </div>

    <div className='cent1'>
      
    </div>

    <div className='chair2'>
    <img src='Chair2.jpg' alt='chair' className='chaiir1' alt'chair2'></img>

    </div>

    </div>

    <div className='rightcont'>
    <div className='right'>
    
    <ul>
      <li><a href='store'>Our Stores</a></li>
      <li><a href='blog'>Blog</a></li>

        
      </ul>
    </div>  

    <div className='chair3'>
    <img src='Chair3.jpg' alt='chair' className='chaiir1'></img>
      
    </div>

    </div>
  
  </div>
  );
}

export default App;
